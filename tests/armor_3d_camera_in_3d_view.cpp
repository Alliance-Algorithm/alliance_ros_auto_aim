...
static inline void armor_3d_camera_in_3d_view(
    ...
    const world_exe::interfaces::IArmorInCamera& armor_to_pub, const world_exe::enumeration::ArmorIdFlag& armor_id,
    void (*process)(
...
private:
    void timer_callback() {
        ...
        // world_exe::ros::ArmorMarkerGenerator::generate(armor_interface, armor_id, frame_id, marker_array);
        // a static func ,return visualization_msgs::msg::MarkerArray

        auto test_armor = test_armor_generate();

        world_exe::ros::ArmorMarkerInCameraSpacing armor_makrer_camera_spacing;
        armor_makrer_camera_spacing.set_frame_id("/auto_aim/camera_spacing"); // set it first, or can't visable
        armor_makrer_camera_spacing.set_color(0.0, 1.0, 0.0, 1.0);            // not necessary, default color green
        armor_makrer_camera_spacing.set_time_stamp(rclcpp::Clock().now());    // not necessary, defalue time now
        // visualization_msgs::msg::MarkerArray armor_marker = armor_makrer_camera_spacing.generate(test_armor);

        visualization_msgs::msg::MarkerArray msg{armor_makrer_camera_spacing.generate(test_armor)};
    ...
    void tf_broadcast() {
        ...
        tf.child_frame_id  = "/auto_aim/camera_spacing";
...
static std::vector<world_exe::data::ArmorCameraSpacing> test_armor_generate() {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        single_armor_.position = Eigen::Vector3d(1.0 + 0.25 * sin(angle), 1.0 + 0.25 * cos(angle), 0.10);
