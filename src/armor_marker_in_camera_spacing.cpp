...
class ArmorMarkerInCameraSpacing::Impl {
public:
  Impl() {
    color_.r = 0.0;
    color_.g = 1.0;
    ...
    small_armor_.x = 0.020;
    small_armor_.y = 0.135;
    small_armor_.z = 0.125;

    large_armor_.y = 0.230;
    large_armor_.x = 0.020;
  ...
  generate(const std::vector<data::ArmorCameraSpacing> &armors_data) {
    if (frame_id_.empty()) {
    ...
    for (const auto &armor : armors_data) {
      ...
      if (armor.id == ArmorIdFlag::Unknow && armor.id == ArmorIdFlag::None) {
        continue;
      } else if (armor.id == ArmorIdFlag::Hero &&
                 armor.id == ArmorIdFlag::Base) {
...
ArmorMarkerInCameraSpacing::ArmorMarkerInCameraSpacing()
    : pimpl_(std::make_unique<Impl>()) {}

ArmorMarkerInCameraSpacing::~ArmorMarkerInCameraSpacing() = default;

void ArmorMarkerInCameraSpacing::set_frame_id(const std::string &frame_id) {
  pimpl_->set_frame_id(frame_id);
}

std::string ArmorMarkerInCameraSpacing::frame_id() {
  return pimpl_->frame_id();
}

void ArmorMarkerInCameraSpacing::set_color(float r, float g, float b, float a) {
  pimpl_->set_color(r, g, b, a);
}

void ArmorMarkerInCameraSpacing::set_time_stamp(const rclcpp::Time &time) {
  pimpl_->set_time_stamp(time);
}

visualization_msgs::msg::MarkerArray ArmorMarkerInCameraSpacing::generate(
